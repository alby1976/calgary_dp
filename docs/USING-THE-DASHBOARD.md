...
- Select **Filters** beside the search box to open the filter drawer without shrinking the permit list. On a desktop it overlays the right side of the page; on a narrow screen it opens as a bottom sheet. Changes update the permit list and both maps immediately. Close it with **Show matches**, the close button, the shaded background or the `Esc` key.
- **Years**, **Permit statuses**, **Land-use districts**, **Permitted / discretionary** and **Appeal statuses** are all multi-select menus inside the drawer. Every value starts selected. Open a menu and uncheck as many values as you do not want to see; its selected-count summary updates immediately. **Select all** restores every value in that menu, while **Deselect all** removes every listed value.
- The number on **Filters** reports how many filter categories have exclusions. Removable chips below the toolbar summarize those categories; selecting a chip restores every value in that category. Search text remains visible in the search box rather than becoming a chip.
...
- **Set current as default** in the drawer saves the search text and all five filter selections in this browser. The saved default is automatically restored on the next visit in the same browser. It is not sent to the server or shared with other browsers or devices. Use **Restore saved default** after temporary changes, **Forget saved default** to remove it immediately, or clear the site's cookies and browser data.
...
Select **Permit field value guide** to open the searchable guide in a right-side drawer without leaving the dashboard or lengthening the permit-details panel. On a narrow screen it opens as a bottom sheet. Close it with either **Close guide**, the close button, the shaded background or the `Esc` key. The **Land-use district** entry lists the district codes found in the loaded Varsity records beside the descriptions published in the City feed. It also explains common `d`, `f` and `h` modifiers and flags Direct Control districts as site-specific. The **Permitted / discretionary** entry explains each loaded classification, including permitted with a relaxation and unspecified values. The **Current status** entry gives a cautious plain-language meaning for every status found in those records. These lists update as City values change; an unfamiliar future value remains visible with advice to check the official application.

Select the **?** control beside any field to open the guide drawer at that definition. The drawer also includes a search box for browsing all definitions. Each help control has a 44×44 CSS-pixel target and moves keyboard focus to the matching definition. Closing the drawer returns focus to the control that opened it. The explanations are plain-language summaries; official City and SDAB records remain authoritative.
