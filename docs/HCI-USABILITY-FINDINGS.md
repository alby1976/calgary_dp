...
| Medium | The complete selected-permit article remains `aria-live="polite"` and `aria-atomic="true"`; the new pinned-selection note also uses `role="status"`. | Selecting an older permit may produce both a concise pin message and a long announcement of dates, metadata, explanations and links. | Remove the live region from the full details article and use one short dedicated status message such as “DP2025-05349 selected; details updated.” | One concise announcement occurs per selection; users can then navigate the details normally without the entire article being reread. |
