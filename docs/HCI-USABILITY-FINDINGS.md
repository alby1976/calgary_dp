...
| High | Selecting a map point outside the latest 12 records updates both maps and the details but leaves no selected row visible in the explorer. After **Show all**, the tested row existed roughly 126,955 pixels below the list viewport and the list did not scroll to it. | The central linked-selection promise breaks at the explorer. Users can lose the list context for the record they selected from a map. | Keep the selected permit in the displayed explorer subset or reveal and scroll the selected row into view when selection originates outside the visible list. Preserve the user's surrounding context where possible. | After selecting any map point, the matching explorer row is rendered, visibly selected and inside the explorer viewport without requiring manual search through the complete list. |
