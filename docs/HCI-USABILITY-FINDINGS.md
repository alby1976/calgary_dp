...
| High | Map points and legend controls remain visually small. | The controls can be difficult to acquire with touch, tremor, low vision or an imprecise pointer, especially when records share nearby coordinates. | Give legend entries at least a 24×24 CSS-pixel target, preferably 44×44; provide zoom and explorer alternatives for dense map points. | Target-size checks pass, nearby points can be separated by zoom, and every record remains selectable from the explorer. |
...
2. ~~Reduce keyboard tab stops without hiding record-level points.~~ Completed August 11, 2026 with canvas point layers and the explorer keyboard path; target-size review remains open.
