...
**Drawer-layout implementation update:** August 18, 2026 — the five persistent categorical menus and browser-default controls moved into an overlay drawer, leaving the permit explorer's normal flow for search, active-filter summaries and more permit rows. The searchable permit field value guide now uses the same overlay pattern instead of lengthening the selected-permit panel. Both drawers become bottom sheets on narrow screens, support `Escape` and return focus to the control that opened them. These changes have automated regression coverage but have not yet received a new hands-on usability retest.
